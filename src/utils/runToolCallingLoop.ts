// Steps 1 + 6: Standalone tool calling loop using toolRegistry
...
export type CompletionOverrides = {
  n_predict?: number;
  temperature?: number;
  top_p?: number;
  stop?: string[];
...
/**
 * Run the multi-turn tool calling loop. No React hooks — pure async function.
 * Used by both the in-app hook and the AIDL service proxy.
 */
export async function runToolCallingLoop(
  ...
  for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
    ...
    console.warn('[ToolCallingLoop] Iteration', iteration);
    ...
    // First check native tool_calls (future-proof)
    let toolCalls = (result as any).tool_calls as ToolCallRaw[] | undefined;
    ...
    for (const tc of toolCalls) {
      ...
      console.warn('[ToolCallingLoop] Tool', tc.function.name, '→', toolResult);
